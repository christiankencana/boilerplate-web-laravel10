            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">
                  <a href="{{ route('companies.locations.index', $company->id) }}" class="btn btn-info">Set Location</a>
                </td>
                @endif
                @if(auth()->user()->hasAnyPermission(['companies.edit']) || auth()->user()->hasAnyPermission(['companies.delete']))
                <td class="text-center">
                  <!-- <a href="{{ route('companies.show', $company->id) }}" class="btn btn-info">View</a> -->
                  @if (!auth()->user()->companies->contains($company) && auth()->user()->hasAnyPermission(['companies.edit']))
                  <a href="{{ route('companies.edit', $company->id) }}" class="btn btn-warning">
                    <i class="small-icon" data-feather="edit"></i>
                  </a>
                  @endif
                  @if(!auth()->user()->companies->contains($company) && auth()->user()->hasAnyPermission(['companies.delete']))
