  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-body">
        ...
        <a href="{{ route('companies.locations.create', ['company' => $company->id]) }}" class="btn btn-primary mb-3 float-right">
          ...
          <table id="dataTableExample" class="table">
            <thead>
              <tr class="text-center">
                <th>Name</th>
                <th>Address</th>
                @if(auth()->user()->hasAnyPermission(['companies.edit']) || auth()->user()->hasAnyPermission(['companies.delete']))
                <th>Actions</th>
                @endif
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">
                  ...
                  <a href="{{ route('companies.locations.edit', ['company' => $company->id, 'location' => $location->id]) }}" class="btn btn-warning">
                    <i class="small-icon" data-feather="edit"></i>
                  </a>
                  @endif
                  @if (auth()->user()->hasAnyPermission(['companies.delete']))
                  <form action="{{ route('companies.locations.destroy', ['company' => $company->id, 'location' => $location->id]) }}" method="POST" style="display: inline-block;">
                    @csrf
                    @method('DELETE')
