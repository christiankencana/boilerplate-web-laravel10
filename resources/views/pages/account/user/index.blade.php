                                ...
                                @foreach ($users as $user)
                                    <tr>
                                        <td class="text-center">
                                            <img src="{{ $user->profile_photo ? Storage::url($user->profile_photo) : asset('profile-pic.png') }}"
                                        ...
                                        <td class="text-center">
                                            @if (auth()->user()->hasAnyPermission(['users.edit']))
                                                <form action="{{ route('users.updateStatus', $user->id) }}" method="POST"
                                                    ...
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="checkbox" name="status_account" value="active"
                                                        onchange="this.form.submit()"
                                                        {{ $user->status_account == 'active' ? 'checked' : '' }}>
                                                </form>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            ...
                                            <button class="btn btn-sm btn-info" data-toggle="modal"
                                                data-target="#rolesModal-{{ $user->id }}">Set Roles</button>
                                        </td>
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-info" data-toggle="modal"
                                                data-target="#companiesModal-{{ $user->id }}">Set Companies</button>
                                        </td>
                                        <td class="text-center">
                                            @if (auth()->user()->hasAnyPermission(['users.edit']))
                                    ...
                                    <!-- Roles Modal -->
                                    <div class="modal fade" id="rolesModal-{{ $user->id }}" tabindex="-1"
                                        aria-labelledby="rolesModalLabel-{{ $user->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-scrollable">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="rolesModalLabel-{{ $user->id }}">Set
                                                        Roles for : {{ $user->name }}</h5>
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('users.updateRoles', $user->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('POST')
                                                        <div class="form-group">
                                                            <small for="roles"
                                                                class="form-text text-muted text-italic">Select multiple
                                                                roles using ctrl + left click</small>
                                                        </div>
                                                        <div class="form-group">
                                                            <select class="select2-multiple" id="roles" name="roles[]"
                                    ...
                                    <div class="modal fade" id="companiesModal-{{ $user->id }}" tabindex="-1"
                                        aria-labelledby="companiesModalLabel-{{ $user->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-scrollable">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="companiesModalLabel-{{ $user->id }}">
                                                        Set Companies for : {{ $user->name }}</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('users.updateCompanies', $user->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('POST')
                                                        <div class="form-group">
                                                            <small for="companies"
                                                                class="form-text text-muted text-italic">Select multiple
                                                                companies using ctrl + left click</small>
                                                        </div>
                                                        <div class="form-group">
                                                            <select class="select2-multiple" id="companies"
                                                                name="companies[]" style="width: 100%" multiple>
